...
to run as a 3+3 design while performing comparably to model-based designs.
...
```{r}
boin_decision_table(target = 0.30, max_n = 12)[1:6, ]
```
...
Four options change how conservative the design is.
...
`n_earlystop` stops a trial once the current dose has accrued enough patients
for the algorithm to have settled. It defaults to 18. Note that the reference
...
The engine consumes one uniform random variate per patient, in enrollment order,
and applies the decision rules in the order used by `BOIN::get.oc()`. With the
